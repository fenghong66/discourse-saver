  ...
  const SaveModule = (function() {
    ...
    async function sendToObsidian(markdown, fileName, config) {
      ...
      if (config.useAdvancedUri) {
        // Advanced URI 插件支持 mode=overwrite 实现覆盖
        const parts = [];
...
<body>
  ...
  const UIModule = (function() {
    ...
    function showSettingsPanel() {
      ...
      overlay.innerHTML = `
        <div class="ds-settings-panel">
          <h2>📝 Discourse Saver 设置 (V4.5.0)</h2>
    ...
    function init() {
      ...
      console.log('[Discourse Saver] 油猴脚本已加载 (V4.5.0)');
    }
